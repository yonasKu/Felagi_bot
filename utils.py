...
def setup_logger(name):
    ...
    error_handler = logging.FileHandler(
        f"logs/errors_{current_date}.log", encoding="utf-8"
    )
        ...
        with open(LOCATIONS_FILE, "r", encoding="utf-8") as file:
            ...
            for location in locations:
                if not validate_location_data(location):
                    logger.warning(
                        f"Invalid location data: {location.get('name', 'Unknown')}"
                    )
            return locations
...
def get_nearby_places(
    latitude: float, longitude: float, max_distance: int = RADIUS_SEARCH
) -> tuple:
    ...
    try:
        ...
        for place in data.get("locations", []):
            ...
            if coords:
                try:
                    ...
                    logger.warning(
                        f"Invalid coordinates for place {place.get('name')}: {ve}"
                    )
                    continue
        ...
        if not nearby_places:
            if max_distance:
                logger.info(
                    f"No places found within {max_distance}m of coordinates ({latitude}, {longitude})"
                )
            else:
        ...
        logger.info(
            f"Found {found_places} places{f' within {max_distance}m radius' if max_distance else ''}"
        )
...
async def show_results(update, places, category=None):
    ...
    try:
        if not places:
            ...
            await update.message.reply_text(
                "No places found.", reply_markup=InlineKeyboardMarkup(keyboard)
            )
            return
...
def debug_log(update: Update, handler_name: str):
    ...
    try:
        ...
        if update.callback_query:
            logger.debug(
                f"{handler_name}: Callback from User {user.id} - {update.callback_query.data}"
            )
        elif update.message:
            logger.debug(
                f"{handler_name}: Message from User {user.id} - {update.message.text}"
            )
    except Exception as e:
